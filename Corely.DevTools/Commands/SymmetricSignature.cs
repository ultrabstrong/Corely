...
namespace Corely.DevTools.Commands
{
    internal class SymmetricSignature : CommandBase
    {
        ...
        [Argument("Key to validate (default), sign message, or verify signature (-s flag)", false)]
        ...
        private string SignatureTypeCode { get; init; } = null!;
        ...
        {
            var signatureProvider = _signatureProviderFactory.GetProvider(SignatureTypeCode ?? DEFAULT_SIGNATURE_TYPE);
        ...
        private void Sign()
        {
            ...
            var signature = _signatureProviderFactory
                .GetProvider(SignatureTypeCode ?? DEFAULT_SIGNATURE_TYPE)
